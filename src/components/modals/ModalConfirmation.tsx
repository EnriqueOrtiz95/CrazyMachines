...
import { IModalConfirmation } from "./IModalConfirmation";
        ...
        <div className="h-screen w-full fixed left-0 top-0 flex justify-center items-center bg-black bg-opacity-50 rounded-lg overflow-y-scroll">
          <div className="bg-gray-form2 rounded shadow-lg max-w-[85%] w-[700px] mt-[90px] p-12 relative">
            ...
            <a
              ...
              onClick={() => {
                setIsOpen(false);
                if (title == "User already exists" && setUserExists) {
                  setUserExists(false);
                  return;
                }
                if (title == "Codigo Enviado!" && setResendCode) {
                  setResendCode(false);
                  return;
                }
                if (title == "Registro Exitoso!" && setRegisterDone) {
